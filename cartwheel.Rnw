...
(M\&V\nomenclature{M\&V}{Measurement \& Verification}).
...
(IPMVP\nomenclature{IPMVP}{International Performance Measurement and Verification Protocol})
...
air-handling unit each. The energy savings were determined by simulation
and by retrofitting one room while leaving another one unchanged.
Both approaches yielded roughly 30\% energy savings on the heating
coil's consumption, but the simulation underestimated by a factor
5 the savings on the fans' electrical consumption\citep{Ginestet2010}. 
...
\citep{Kaiser2010,Cohen1991}, or ignored \citep{Ginestet2010,Lin2015}.
...
balance the building's heat loss. The daily losses $Q_{d}$\nomenclature{$Q_{d}$}{energy demand on day $d$}
on a day $d$ can be modeled as a random variable drawn from a Gaussian
distribution, whose mean is proportional to the positive difference
\nomenclature{$(.)^{+}$}{positive part}between the average outdoor
temperature and the building's \emph{base temperature}: 
\begin{equation}
Q_{d}=K\times(\tb-\tout)^{+}+\mathcal{N}(0,\sigma^{2})
\end{equation}
\nomenclature{$\mathcal{N}(0,\sigma^{2})$}{Gaussian white noise of mean 0 and variance $\sigma^2$}
where the base temperature $\tb$ \nomenclature{$\tb$ }{base temperature}of
a building is defined as the outdoor temperature above which all heat
losses are compensated by the sun and other free gains, and $\tout$
\nomenclature{$\tout$}{mean outdoor temperature on day $d$} is the
average outdoor temperature on day $d$. The base temperature depends
on the indoor temperature's set point. The building's total heat loss
coefficient $K$\nomenclature{$K$}{total heat loss coefficient}is
the quantity of extra heat required per day to maintain indoor comfort
for each extra degree of cold. It is the most important metric when
assessing the efficacy of a new heating control algorithm, because
the base temperature $\theta_{\text{b}}$ will only change with the
indoor temperature set point.

Over a period $p$ made up of $n_{p}$ \nomenclature{$n_{p}$}{number of days in period $p$}days,
the total heating load \nomenclature{$Q_{p}$}{energy demand during reporting period $p$}will
be 
\begin{equation}
Q_{p}=\sum_{d\in p}Q_{d}=K\times DD_{p}+\mathcal{N}(0,n_{p}\sigma^{2})\label{eq:Qp}
\end{equation}
where $DD_{p}=\sum_{d\in p}(\tb-\tout)^{+}$ \nomenclature{$DD_{p}$}{heating degree-days during period $p$}is
the number of \emph{degree-days at base temperature $\tb$} during
that period. $Q_{p}$ is frequently available as utility bills or
energy meter readouts, while $DD_{p}$ must be calculated from historic
weather data and the building's estimated base temperature.
...
Letting $D$ \nomenclature{$D$}{set of energy consumption data}stand
...
consists in establishing the probability density function for the
coefficients $\{K,\theta_{\text{b}},\sigma\}$ conditioned on the
information $D$, noted as $p(\{K,\theta_{\text{b}},\sigma\}\mid D)$.
...
$\rho$\nomenclature{$\rho$}{factor by which the total heat loss coefficient has changed},
and also possibly shift the base temperature if the indoor temperature's
set points are not exactly identical before and after the installation:
\[
...
oilConsumptionPeriodsTable <- 
  oilConsumptionPeriods %>%
    ...
    xtable(caption = "Usable pre-retrofit consumption periods", label = "tab:oilperiods", digits = c(0, 0, 0, 0, 0, 1, 1), align = c("X", "X", "X", "X", ">{\\hsize=0.5\\hsize}X", "X", ">{\\hsize=1.5\\hsize}X"))
...
\textcelsius , and $K_{\mathrm{pre}}=\Sexpr{p(Kpre)}\pm\Sexpr{p(deltaKpre)}$
L/K.
...
<<OilVsDD, fig.cap="Oil consumption vs heating degree days, calculated with different base temperatures. Unfilled circles indicate the two records that have been excluded from the analysis. The Bayesian base temperature yields the highest coefficient of determination $R^2$, suggesting that it fits the data best.", fig.width=one_and_half_columns, fig.height=4, fig.env="figure*">>=
...
L/K, or about 30\% less than the pre-installation total heat loss
...
<<RhoDensity, fig.cap="Probability density distribution for the relative energy savings in percent.">>=
