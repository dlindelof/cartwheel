...
\documentclass[twoside,english,review, sort&compress]{elsarticle}
...
found to have improved by 33.1\%, with 19.0 percentage points standard
error. 
\end{abstract}
\begin{keyword}
...
have publicly committed to reducing their greenhouse gas emissions.
...
temperature, and annual electricity savings was reduced by 3\,853\,625\,kWh,
...
here consisted in using cross-validation to eatimate the uncertainty
...
is an estimate of the model's uncertainties\citet{Walter2014}.
\end{itemize}
...
about 16'000 buildings in Switzerland, of which about 950 are sales,
office, and non-production buildings. They are one of the largest
building owner in Switzerland in number of buildings. Their yearly
...
CO\textsubscript{2} generated by our entire operations and supply
chain'' for themselves and their customers by 2020, and to ``increase
our own energy efficiency by a further 35\% between 2016 and 2020''
\citep{swisscom2016}. Like many major Swiss companies, their stock
of buildings includes a majority of systems heated by non-renewable
fuel. Energy conservation measures that help these buildings become
more efficient are particularly attractive. In late 2015, a pilot
study was started to evaluate the effectiveness of model-predictive
...
is refilled. The domestic hot water is provided by an independent
circuit.
...
the sun and other free gains. This temperature generally depends on
the indoor temperature's set point. The building's total heat loss
coefficient $K$ is the quantity of extra heat required per day to
maintain indoor comfort for each extra degree of cold. It is the most
important metric when assessing the efficacy of a new heating control
algorithm, because the base temperature $\theta_{\text{b}}$ should
remain unaffected by a new control logic.
...
from now on the data from Bad Ragaz. 
...
<<OilVsDD, fig.cap="Oil consumption vs heating degree days, calculated with different base temperatures. Solid circles indicate the two records that have been excluded from the analysis. The Bayesian base temperature yields the highest coefficient of determination $R^2$, showing that it fits the data best.">>=
...
ggplot(before.dd.long, aes(`Degree days`, Oil, shape = Good)) +
	...
	geom_text(data = before.rsq, aes(1200, 2500, label = paste("italic(R)^2=='", label, "'"), hjust = 0), parse = TRUE, size = rel(3), inherit.aes = FALSE) +
	ylab("Oil [L]") +
...
and they all are lower than the base temperature. We obtain $K_{\mathrm{post}}=\Sexpr{p(Kpost)}\pm\Sexpr{p(deltaKpost)}$
