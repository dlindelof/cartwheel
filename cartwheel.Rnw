...
fans' electrical consumption\citet{Ginestet2010}. 
...
ranging from -\$515 to \$530\citet{Kaiser2010}. 
\end{itemize}
...
oilConsumptionPeriodsTable <- 
  oilConsumptionPeriods %>%
    ...
    dplyr::rename(From = from, To = to, "Oil [L]" = oil, Days = nDays, "Avg. temp. [C]" = meanTemp) %>%
    xtable(label="tab:oilperiods")
...
installation can now be evaluated, defined as $\rho=(1-K_{\mathrm{post}}/K_{\mathrm{pre}})$.
From a Bayesian point of view, $\rho$ is a function of two random
variables $\kpre$ and $\kpost$ and we need its pdf. This can be
estimated from random draws of $\kpre$ and $\kpost$, evaluating
