...
finding the \emph{probability distribution functions} of these parameters
\citep{Gelman2013}.
...
But unlike that prior work, where the focus was on the determination
of a building's base temperature, here we will perform this estimation
...
pre-installation data, and vice-versa. Once we have the pdf on the
left-hand side of the equation, inferences about $\rho=K_{\text{post}}/K_{\text{pre}}$
