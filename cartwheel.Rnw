...
\documentclass[twoside,english,3p, sort&compress]{elsarticle}
...
<<>>=
fit.before <- bhm(Oil ~ DailyMeans, subset(goodPeriods, When == "PRE"), baseLoad = 0)
...
fit.after <- bhm(Oil ~ DailyMeans, subset(goodPeriods, When == "POST"), baseLoad = 0)
...
from this data using our method. We obtain $t_{\text{b}}=\Sexpr{p(baseTempPre)}\pm\Sexpr{p(deltaBaseTempPre)}$
