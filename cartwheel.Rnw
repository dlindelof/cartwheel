...
model-predictive controller for space heating for a medium-sized (about
2\,700\,m\textsuperscript{2}) office building in Switzerland. The
baseline was established from historic oil tank refill records. The
energy efficiency of the building was assessed again 55 days after
the installation and found to have improved by 33.1\%, with 19.0 percentage
...
from trivial (replacing light bulbs) to major (better façade insulation),
an owner will want to know which energy conservation measure is likely
...
will want to know how well it performs and whether their money was
well spent\textemdash an activity known as Measurement \& Verification
(M\&V).

This is a recurring problem for large companies in Switzerland that
have publicly committed to specific goals for reducing their greenhouse
gas emissions. The majority of these emissions can be traced to large
portfolios of legacy buildings \citep{BFE2013b}, in particular to
the fossil fuels used for heating. Reducing the heating energy for
legacy buildings (while maintaining user comfort) will help such companies
achieve their objectives\textemdash provided those energy savings
can be proven. Given a choice between ECMs, a facility manager will
frequently favor those perceived as being of relatively low-risk,
even when their effectiveness might be less attractive.
...
suffers from an image of (bad) science, or, as \citeauthor{Xia2013}
put it, of ``an inaccurate science: an engineering practice relying
heavily on professional judgement'' \citep{Xia2013}. We must show
building owners that assessing the effectiveness of ECMs is not a
research project anymore but a well-defined and well-understood process,
one that yields not just the estimated energy savings but also their
...
(IPMVP) \citep{evo2012} is a widely recognized framework for conducting
M\&V projects. It provides a set of guidelines and documents common
terms and methods for such projects. Energy savings are to be determined
...
demand, taking into account external factors that influence the energy
demand:
\begin{align*}
...
A Scopus search for ``IPMVP'' in a document's title, abstract or
keywords, limited to publication date from 2010 to present, yields
...
period ran from August 2013 to July 2014, and a $t$-test was used
to determine whether the energy savings were significant. Very different
results were obtained depending on which variables were used to model
the baseline energy consumption, and the relative uncertainty on the
energy savings ranged from 86\% to 232\% \citep{Aris2015}.
\item Two near-identical 300-seat conference rooms retrofitted with a variable-flow
AHU each. The energy savings were determined by simulation and by
retrofitting one room while leaving another one unchanged. Both approaches
yielded roughly 30\% energy savings on the heating coil's consumption,
but the simulation underestimated by a factor 5 the savings on the
fans' electrical consumption\citep{Ginestet2010}. 
...
model in HDD, CDD, relative humidity and energy price on 52 households
out of the 730 households that participated in 2003. An average of
172 kWh annual energy savings was found, with monetary annual savings
ranging from -\$515 to \$530\citep{Kaiser2010}. 
\end{itemize}
...
temperature, and annual electricity consumption was reduced by 3\,853\,625\,kWh,
although no uncertainty was reported for this extraordinarily precise
figure \citep{Lin2015} .
\item An analysis of the electric load of 17 office buildings, modeled as
a piece-wise linear function of time of week and outdoor temperature,
for the explicit purpose of establishing a baseline model. The novelty
here consisted in using cross-validation to estimate the uncertainty
in the baseline: the last month of baseline data was excluded from
the model, and compared with the model's prediction. The difference
is an estimate of the model's uncertainties \citep{Walter2014}.
\end{itemize}
...
assume an arbitrary base temperature for HDD calculation \citep{Kaiser2010,Cohen1991},
or ignored \citep{Ginestet2010,Lin2015}. \citet{Walter2014} have
proposed a method that is a step towards making energy savings uncertainty
estimation more reliable, but they concede that the method is limited
by the amount of data available.

In this paper we describe a method based on Bayesian data analysis,
that will yield correct uncertainties no matter how much data is available.
...
their stock of buildings includes a majority of systems heated by
non-renewable fuel. Energy conservation measures that help these buildings
become more efficient are particularly attractive. In late 2015, a
pilot study was started to evaluate the effectiveness of model-predictive
...
\citep{neurobat2017} was selected for a pilot study on a building
in Sargans, a Swiss locality in the canton of St Gall. Like many buildings,
the pre-installation energy efficiency of this building was not well
understood by the building's owner, and the project plan did not allow
for the measurement of a baseline performance. All that was available
was a record of oil tank refills, stretching back to 2011. The poor
baseline data made Bayesian methods particularly attractive for this
(and similar) projects.

In this paper we will first describe the test installation (section
\ref{sec:Experimental-building}), followed by a review in section
\ref{sec:Bayesian-energy-model} of the Bayesian data analysis method
we employed to establish the building's baseline performance and post-installation
performance. The weather, baseline period and reporting period data
will be analysed in section \ref{sec:Data-analysis}, and inferences
about the energy savings will be drawn in section \ref{sec:Results}.
...
four heating circuits, covering about 2700\textendash 2800 m\textsuperscript{2},
...
is about 1300\textendash 1400 m\textsuperscript{2}.
...
The NOL\textsuperscript{a} system implements a model-predictive controller
that regularly computes an optimal water supply temperature. It therefore
belongs to the ECM category of ``Energy efficient equipment and low
energy technologies'', a form of demand side management \citep{Ma2012}.
...
each circuit a heating water at just the right temperature. As this
paper is about the estimation of energy savings resulting from an
ECM, the details of the NOL\textsuperscript{a} algorithms are beyond
the scope of this article but can be found elsewhere \citep{Bauer98,Zimmermann1985,Morel2001161,Krauss200863,Bichsel2000,NoAuthor1998,Lindelof2015}. 
...
\section{Bayesian energy model\label{sec:Bayesian-energy-model}}
...
\end{equation}
 where the base temperature $\tb$ of a building is defined as the
outdoor temperature above which all heat losses are compensated by
the sun and other free gains, and $\tout$ is the average outdoor
temperature on day $d$. The base temperature depends on the indoor
temperature's set point. The building's total heat loss coefficient
$K$ is the quantity of extra heat required per day to maintain indoor
comfort for each extra degree of cold. It is the most important metric
when assessing the efficacy of a new heating control algorithm, because
the base temperature $\theta_{\text{b}}$ will only change with the
...
\end{equation}
where $DD_{p}=\sum_{d\in p}(\tb-\tout)^{+}$ is the number of \emph{degree-days
at base temperature $\tb$} during that period. $Q_{p}$ is frequently
available as utility bills or energy meter readouts, while $DD_{p}$
...
model from which that data is drawn. Bayesian inference consists in
finding the parameters of that probability model that are the most
consistent with the observed data, yielding probability distribution
functions of these parameters \citep{Gelman2013}.
...
as implemented in a package for the R programming environment \citep{homeR}.
Unlike that prior work, where the focus was on the determination of
a building's base temperature, here we will be more interested by
...
$\rho$, and also possibly shift the base temperature if the indoor
temperature's set points are not exactly identical before and after
the installation: 
\[
K_{\mathrm{pre}}\rightarrow K_{\mathrm{post}}=\rho\times K_{\mathrm{pre}}
\]
\[
\theta_{\text{b, pre}}\rightarrow\theta_{\text{b, post}}=\theta_{\text{b, pre}}+\Delta t_{\text{b}}
\]
...
In IPMVP terminology, the routine adjustments will be due to outdoor
conditions and are covered by $K$, while non-routine adjustments
will be dealt with by taking $\Delta t_{\text{b}}$ into account.
...
\shoveright{\times p(\{K_{\text{post}},\theta_{\text{b,post}},\sigma_{\text{post}}\}\mid D)}\\
...
proposed by \citet{Lindelof2016}. Once we have the pdf on the left-hand
side of the equation, inferences about $\rho=K_{\text{post}}/K_{\text{pre}}$
...
be explained in the following section, followed by the analysis of
...
from oil meters were available only after the installation of the
NOL\textsuperscript{a}. The energy savings must therefore be derived
from heterogenous sources of data: manual oil level readouts before
the installation, and automated daily oil readouts after.
...
The data from the stations is shown against time in Fig.~\ref{fig:ClimatePlot}.
...
<<ClimatePlot, fig.cap="Historic outdoor temperature measured at nearby weather stations. There is no obvious disagreement between the stations.", fig.width=3.5, fig.height=3>>=
...
<<ClimateScatterPlot, fig.cap="Comparison of Bad Ragaz and Vaduz temperatures.", fig.width=3.5>>=
...
each period a list of mean daily outdoor temperatures, each list as
long as the period. We obtain the data shown in Table \ref{tab:oilperiods}.
...
oilConsumptionPeriodsTable <- 
  oilConsumptionPeriods %>%
    ...
    dplyr::rename(From = from, To = to, "Oil [L]" = oil, Days = nDays, "Avg. daily cons. [L]" = meanCnsmptn, "Avg. temp. [C]" = meanTemp) %>%
    xtable(caption = "Usable pre-retrofit consumption periods", label = "tab:oilperiods", digits = c(0, 0, 0, 0, 0, 1, 1))
...
from this data. We obtain $\tb=\Sexpr{p(baseTempPre)}\pm\Sexpr{p(deltaBaseTempPre)}$
C, and $K_{\mathrm{pre}}=\Sexpr{p(Kpre)}\pm\Sexpr{p(deltaKpre)}$
L/K.
...
standards \citep{ashrae2013,CTG075} . The Bayesian method yields
the best results. The relationship between the oil consumption and
the degree days using the estimated base temperature is satisfactorily
linear, with the exception of the two outliers identified above (not
plotted), and possibly a third point with 2300 liters of oil for about
750 degree days. 

<<OilVsDD, fig.cap="Oil consumption vs heating degree days, calculated with different base temperatures. Solid circles indicate the two records that have been excluded from the analysis. The Bayesian base temperature yields the highest coefficient of determination $R^2$, showing that it fits the data best.", fig.width=4>>=
...
before.rsq <- before.dd.long %>%
  ...
  dplyr::mutate(rsq = summary(model)$r.squared,
         K = coef(model)[2]) %>%
  dplyr::select(-model)
before.rsq$label <- format(before.rsq$rsq, di = 2)
before.rsq$Klabel <- format(before.rsq$K, di = 2)

ggplot(before.dd.long, aes(`Degree days`, Oil, shape = Good)) +
	geom_point() +
	facet_wrap(~ Base) +
	geom_smooth(method = "lm", data = subset(before.dd.long, Good), formula = y ~ x, se = FALSE) + 
...
Each point in this plot represents one day between 1st April 2016,
when a mislabeling of the existing heating system was identified and
corrected, and 16 May 2016, just before the end of the heating season.
The post-installation total heat loss coefficient $K_{\mathrm{post}}$
is the slope of the linear regression through these points, since
each point represents one day of oil consumption and they all are
lower than the base temperature. We obtain $K_{\mathrm{post}}=\Sexpr{p(Kpost)}\pm\Sexpr{p(deltaKpost)}$
...
The increase in energy efficiency (and its uncertainty) resulting
from the NOL\textsuperscript{a} installation can now be evaluated,
defined as $\rho=(1-K_{\mathrm{post}}/K_{\mathrm{pre}})$. From a
Bayesian point of view, $\rho$ is a function of two random variables
$\kpre$ and $\kpost$ and we need its pdf. This can be estimated
from random draws from the pdfs of $\kpre$ and $\kpost$, evaluating
$\rho$, and evaluating values of interest such as the mean value,
standard deviation, and so on. 

A histogram of that probability density is shown in Fig.~\ref{fig:RhoDensity}.
The majority of that histogram is positive, indicating positive energy
savings. Any statement about the energy savings can now be made with
its associated confidence. For example, 75\% of the mass of this histogram
lies to the right of 14\% energy savings; this can be reported by
saying that the energy savings are at least 14\% with 75\% probability.
The `best' estimate of the final energy savings is obtained by finding
the narrowest range of energy savings such that they encompass $N$\%
of the mass of the histogram, also known as the $N$\% credible interval,
for any desired value for $N$. Since one standard error of a normal
distribution corresponds to a 67\% credible interval, we will report
the energy savings with the same value for $N$. We obtain:
\begin{center}
...
the experiment run longer. But letting the experiment run longer will
only reduce the error on the post-installation heat loss coefficient
$K_{\text{post}}$; there is nothing we can do about the error on
the pre-installation heat loss coefficient $K_{\text{pre}}$, which
dominates the total error on the final energy savings. The absolute
error on the savings, which are equal to the absolute error on $\rho$,
have a lower limit given by the relative error on $K_{\text{pre}}$:
$\Delta\rho\geq\rho\times\Delta K_{\text{pre}}/K_{\text{pre}}$. In
our case, $\rho=\Sexpr{p(rho)}$, $\Delta K_{\text{pre}}=\Sexpr{p(sd(m[,'Kpre']))}$,
...
days of measurements after the installation, our accuracy is almost
...
The magnitude of the estimated energy savings is comparable with the
uncertainty surrounding them. From a strict interpretation of the
IPMVP text, the energy savings would not be considered statistically
significant. But a glance at Fig.~\ref{fig:RhoDensity} makes it
clear that the energy savings are more likely to be positive than
not. A single point estimate with uncertainty strikes us as inadequate
to convey all the information that's been drawn from the data. On
the other hand, displaying probability density functions to decision
makers is probably not a better idea. Instead, we suggest using this
probability density to answer any question about the energy savings
on that building, such as ``What are the most likely energy savings?'',
``What energy savings can I expect on average?'', or ``How likely
am I to achieve energy savings of at least 20\%?''. The latter kind
of questions are answered by the pdf's cumulative distribution function.
For example, Table~\ref{tab:probs} shows for any level of energy
savings, how likely one is to achieve at least that level on this
building. It is essentially a table of cumulative distribution values.
...
print(xtable(savingsProbability, 
             label = "tab:probs",
             digits = c(0, 0, 1),
             caption = "Probability that the true energy savings are higher than a given level."), include.rownames = FALSE, comment = FALSE)
@
...
installer relies on the temperature set point programmed on the original
...
This estimate was obtained by fitting a probability model to the pre-
and post-retrofit oil consumption data, whose parameters include the
building's total heat loss coefficient and its base temperature. This
probability model yielded a probability density function in all the
building's parameters, from which the energy savings were estimated
by repeated draws of the pre- and post-retrofit heat loss coefficients.

We have used a Bayesian analysis framework throughout this analysis
and found it superior to its classical counterpart on at least three
points:
\begin{itemize}
\item its ability to handle a complex probability model, such as the one
relating the daily energy consumption to the mean outdoor temperature
\item its ability to take into account data from very different sources
(pre-retrofit manual entries and post-retrofit automated daily readouts)
\item the clear interpretability (and communicability) of its results
\end{itemize}
Because of this choice, we were able to avoid a costly pre-retrofit
period of measurements. The owner of the building was aware that the
precision of the energy savings was going to be limited by the accuracy
of the pre-retrofit manual readouts, but appreciated the efforts made
to communicate them in a way easy to understand for decision-makers.

Not all buildings will enjoy the luxury of automated energy consumption
readouts, however, and it is likely that future projects will have
no choice but to rely on manual readouts both before and after the
retrofit. The Bayesian framework will likely be able to handle these
situations, but with an interesting twist: it will probably be possible
to pool both the pre- and post-retrofit data to better estimate the
daily energy consumption variability, which we did not do because
...
Switzerland - Single Color by FreeVectorMaps.com
...
\begin{figure*}
\begin{centering}
\includegraphics[width=1\linewidth]{img/oil}
\par\end{centering}
