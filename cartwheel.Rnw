...
Bayesian methods have recently been employed by \citet{ChongKheePohLamMatteoPozziJunjingYang2017}
and \citet{Heine2017} who have shown how to infer building parameters
from measurements; perhaps the only flaw in their approach is their
choice of prior distribution, which tended to remain unaffected by
the data. It would perhaps have been better to start with uninformative
priors.
...
gas meter data on 119 buildings by \citet{Meng2017}, who used an
algorithm due to \citet{Paulus2016} and found base temperatures ranging
from 11.6\,\textcelsius{} to 20.5\,\textcelsius , while the ``standard''
base temperature in the UK is 15.5\,\textcelsius{} \citep{CTG075}.
