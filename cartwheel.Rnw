...
will describe a fully Bayesian approach to do so, and compire its
predictions with an approach based on nonlinear regression (the most
credible alternative we can think of).
