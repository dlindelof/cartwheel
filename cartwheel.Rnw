...
\begin{abstract}
Most building owners will eventually consider investing in energy
conservation measures for their buildings. Faced with a variety of
options, ranging from trivial (replacing light bulbs) to major (better
façade insulation), they want to know which energy conservation measure
is likely to yield the highest return on investment\textemdash and
...
with confidence intervals, how effective the retrofit was.
...
total heat loss coefficient of the building was assessed again 46
days after the installation and found to have decreased by 33.1\%,
...
insulation), an owner will want to know which measures are likely
...
of a ``baseline'' model of the building's energy consumption, ideally
...
Lack in confidence in the effectiveness of ECMs hinder their adoption
...
prove. The latter is the goal of M\&V, which currently suffers from
an image of (bad) science, or, as \citeauthor{Xia2013} put it, of
``an inaccurate science: an engineering practice relying heavily
on professional judgement'' \citep{Xia2013}. We must show building
owners that assessing the effectiveness of ECMs is not a research
project anymore but a well-defined and well-understood process, one
that yields not just the estimated energy savings but also their accuracy.
...
for such projects. Energy savings are to be determined by first establishing
...
factors that influence the energy demand:
\begin{align*}
...
non-renewable fuels. Energy conservation measures that help these
buildings become more efficient are particularly attractive. In late
2015, a pilot study was chartered to evaluate the effectiveness of
...
particularly attractive for this project.
...
about the effectiveness of the controller will be drawn in section
\ref{sec:Results}.
...
circuit. As this paper is about the estimation of energy savings resulting
from an ECM, the details of the NOL\textsuperscript{a} algorithms
are beyond the scope of this article but can be found elsewhere \citep{Bauer98,Zimmermann1985,Morel2001161,Krauss200863,Bichsel2000,NoAuthor1998,Lindelof2015}. 
...
balance the heat losses. The daily losses $Q_{d}$\nomenclature[030]{$Q_{d}$}{energy demand on day $d$}
...
\nomenclature[100]{$(.)^{+}$}{positive part}between the average outdoor
temperature and the building's \emph{base temperature}: 
\begin{equation}
Q_{d}=K\times(\tb-\tout)^{+}+\mathcal{N}(0,\sigma^{2}),
\end{equation}
\nomenclature[090]{$\mathcal{N}(0,\sigma^{2})$}{Gaussian white noise of mean 0 and variance $\sigma^2$}where
the base temperature $\tb$ \nomenclature[060]{$\tb$ }{base temperature}of
...
is the average outdoor temperature on day $d$. The base temperature
is equal to the indoor temperature set point, up to a constant offset.
The building's total heat loss coefficient $K$\nomenclature[050]{$K$}{total heat loss coefficient}
is the quantity of extra heat required per day to maintain indoor
comfort for each extra degree of cold. We agreed with the customer
that the effectiveness of the new control system would be given by
changes to $K$, since the base temperature $\theta_{\text{b}}$ might
change if the indoor temperature set point is not the same before
and after the installation.

Over a period $p$ made up of $n_{p}$ \nomenclature[080]{$n_{p}$}{number of days in period $p$}days,
...
density function for the coefficients $\{K,\theta_{\text{b}},\sigma\}$
...
coefficients. The lower $\rho$ is, the more effective is the new
control system. The relative energy savings in percent (ignoring changes
to $\tb$) are then given by $100\times(1-\rho)$.
...
for the base temperature $\theta_{\text{b}}$. But if the pre- and
post-retrofit parameters are independent, then we are effectively
dealing with two independent experiments: one before the installation,
and one after. The Bayesian analysis can therefore be simplified by
considering separately the data before and after the installation.
...
the post-installation consumption data. 
...
from this data. We obtain $\tb=\Sexpr{p(baseTempPre)}\pm\Sexpr{p(deltaBaseTempPre)}$\,\textcelsius ,
and $K_{\mathrm{pre}}=\Sexpr{p(Kpre)}\pm\Sexpr{p(deltaKpre)}$\,L/K.
...
linear, with the exception of the two outliers identified above, and
possibly a third point with 2300 liters of oil for about 750 degree
days. 
...
We obtain $K_{\mathrm{post}}=\Sexpr{p(Kpost)}\pm\Sexpr{p(deltaKpost)}$\,L/K,
or about 30\% less than the pre-installation total heat loss coefficient
$K_{\mathrm{pre}}$. The post-installation base temperature is now
$\theta_{\text{b,post}}=\Sexpr{p(baseTempPost)}\pm\Sexpr{p(deltaBaseTempPost)}\thinspace\text{\textcelsius}$.
...
defined as $\rho=(1-K_{\mathrm{post}}/K_{\mathrm{pre}})$, can now
be evaluated. From a Bayesian point of view, $\rho$ is a function
of two random variables $\kpre$ and $\kpost$ and we need its probability
distribution function. This can be estimated from random draws from
the joint pdfs of $\kpre$ and $\kpost$, calculating $\rho$, and
evaluating values of interest such as the mean value, standard deviation,
and so on. 

A histogram of the $\rho$ values resulting from 20\,000 draws is
...
75\% confidence. The `best' estimate of the effect is obtained by
finding the narrowest range of $\rho$ values such that they encompass
$N$\% of the mass of the histogram, also known as the $N$\% credible
interval, for any desired $N$. Since one standard error of a normal
distribution corresponds to a 67\% credible interval, we agreed with
the building owner that the unceratinty on the effect should be reported
with the same $N$. We obtain:
\[
...
<<RhoDensity, fig.cap="Probability density distribution of the heat loss reduction.">>=
q <- qplot(100 * m[, 'savings'], y = ..density.., geom = "histogram", xlab = "Heat losses reduction [\\%]", ylab = "Probability density")
q + xlim(-100, 100)
@
...
which the effect on the heat loss coefficient were estimated from
that pdf.
...
The Bayesian framework will likely be able to handle these situations,
but with an interesting twist: it will probably be possible to pool
the pre- and post-retrofit data to better estimate the daily energy
