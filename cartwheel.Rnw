...
The IPMVP documents common terms and methods to evaluate performance
of efficiency projects.

Measurement and Verification\textquotedblright{} (M\&V) is the process
of using measurement to reliably determine actual savings 4 created
within an individual facility by an energy management program.

Where judgements are made about uncertain quantities, M\&V procedures
should be designed to under-estimate savings. 

Similarly the word Energy Conservation Measure (ECM) will commonly
be used to mean measures to improve efficiency or conserve energy
or water, or manage demand. 

Establish the users' acceptable savings accuracy during the M\&V Planning
process. 

The objective of M\&V is to reliably determine energy savings. In
order for savings reports to be reliable, they need to have a reasonable
...
Specifically, the savings need to be larger than twice the standard
error (see definition in Appendix B-1.3) of the baseline value. 

Though there is no universal standard for a minimum acceptable R 2
value, 0.75 is often considered a reasonable indicator of a good causal
relationship amongst the energy and independent variables. The R 2
test should only be used as an initial check. Models should not be
rejected or accepted solely on the basis of R 2 . 

The conventional approach is to design sampling to achieve a 90\%
confidence level and \textpm 10\% precision. 
...
will be 
\[
Q_{p}=\sum_{d\in p}Q_{d}=K\times DD_{p}+\mathcal{N}(0,n_{p}\sigma^{2})
\]
...
Appendix. The data from the stations is shown against time in Fig.~\ref{fig:PlotClimate}.
A scatterplot of the temperatures from Bad Ragaz against those from
Vaduz is shown in Fig.~\ref{fig:ScatterPlotClimate}. There are no
