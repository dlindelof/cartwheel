...
retrofit projects we must address three concerns: 1) the risk that
the ECM might not be as effective as advertised, 2) the risk that,
although effective, the benefits will turn out to be difficult to
prove, and 3) the risk of a poor return on investment. The latter
risk is a function not only of the effectiveness of the ECM, but also
of the fluctuating energy prices throughout the lifetime of the building,
and has been addressed by \citet{Kumbaroglu2012}. Risks 1) and 2)
are the goal of M\&V, currently perceived as ``an inaccurate science:
an engineering practice relying heavily on professional judgement''
\citep{Xia2013}. We must show building owners that assessing the
effectiveness of ECMs can be a well-defined and well-understood process,
one that yields not just the estimated energy savings but also their
accuracy, while complying with existing international norms.
...
\section{Estimating the building's heating demand\label{sec:Bayesian-energy-model}}
...
temperature's set point or the activity inside the building).
...
the heat losses. The heat losses on a given day $d$ will, intuitively,
...
is constant, which is of course not true: above the base temperature,
the daily heating energy demand will be stabler than below the base
temperature. But we do not believe the model needs this extra complexity.)

Putting it all together, the daily losses $Q_{d}$\nomenclature[030]{$Q_{d}$}{energy demand on day $d$}
on a day $d$ is proportional to the positive difference \nomenclature[100]{$(.)^{+}$}{positive part}between
...
of the new heating control system would be uniquerely determined from
...
This is partially why one cannot use a ``one size fits all'' base
temperature, as currently recommended by building regulations and
policies. This was conclusively shown by an analysis of four years
of half-hourly gas meter data on 119 buildings by \citet{Meng2017},
who used an algorithm due to \citet{Paulus2016} and found base temperatures
ranging from 11.6\,\textcelsius{} to 20.5\,\textcelsius , while
the ``standard'' base temperature in the UK is 15.5\,\textcelsius{}
\citep{CTG075}.
...
when drawing randomly from the pdf of a parameter of interest, but
excellent algorithms exist to make this process highly efficient.
...
But why minimize the squared error, and not, say, the absolue error?
...
There are other methods for estimating the parameters of this model.
...
In our experience, we have also seen practitioners use variations
of the performance line method where the coefficient of determination
$R^{2}$ is maximized, where the intercept vanishes, or where the
sum of unweighted squared residuals is minimized. Again, all of these
non-Bayesian methods suffer from arbitrary assumptions. It seems to
us that the most credible alternative to a fully Bayesian approach
is a non-linear regression method, described below.
...
Intuitively, all we're doing here is to normalize the heating energy
demand of each period by its number of days. But notice that the daily
fluctuations $\sigma$ do not enter this sum. They must be estimated
from the sample variance of the residuals.
...
\end{itemize}
However, all these caveats (and many more) apply to an equivalent
analysis done with classical statistics, such as non-linear regression.
In fact, we would cautiously suggest that our approach is much more
likely to bring the attention of the analyst to these problems, because
we have seldom seen these points being discussed in prior similar
works.
...
such situations, provided a suitable model is developed\textemdash which
must also be done done for classical approaches anyway.
...
implementation of this method provided by the homeR package \citep{homeR}
...
before and after. That must be done ``by hand'', as was done for
this paper.
...
This paper was prepared with LyX, exported to the Rnw format, and
processed by knitr \citep{knitr17}. The source LyX file and the generated
Rnw file are available from https://github.com/dlindelof/cartwheel.
