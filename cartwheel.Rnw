...
insulation), an owner will want to know which measures will yield
...
When this is difficult, facility managers will frequently prefer conservative
projects, which doesn't help the the company meet its goals.

But proving the effectiveness of an ECM is hard. Many factors influence
...
in absolute terms or in relative. But many M\&V projects ignore the
...
that the effectiveness of the new control system would be quantified
by the decrease of $K$, since the base temperature $\theta_{\text{b}}$
...
the pre- and post-installation parameters should be taken as conditionally
...
the building owner that the uncertainty on the effect should be reported
