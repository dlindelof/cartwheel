...
\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\citet{Aris2015}: retrofit of a water cooling package unit. Baseline
...
\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\citep{Cohen1991}old but relevant: 32 retrofit projects, ranging
in size from three to 30 000 houses. Energy normalised by HDD using
fixed 65 F. Mix of submetered, utility bills.%
\end{minipage}}

\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\citep{Ginestet2010} applies the IPMVP to an existing building equipped
...
\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\citep{Kaiser2010} reports on the results from a large-scale program
...
\noindent\fbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 2\fboxrule}%
\citep{Lin2015}Electricity and gas bill for a 31'000m2 campus whose
...
regularly calculates an optimal value for the tempera- ture of the
water prepared by the oil boiler. This value cannot be communicated
directly to most legacy heating controllers; instead, the NOLa system
controls the flow temperature indirectly by modulating the outdoor
...
I am very grateful to Dr Antoine Guillemin and Sohail Malik for their
helpful review of an early version of this paper, and for their encouragements
in getting this submitted for publication.

\section*{Implementation notes}

An implementation of this method for the R programming environment
\citep{Rcore2015} is provided by the \code{bhm()} function in the
\code{homeR} package, available for download on CRAN \citep{homeR}.
